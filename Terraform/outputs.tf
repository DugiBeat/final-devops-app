...
output "project_summary" {
  value = join("\n", [
    ...
    "-----------------Kbectl get cmd-----------------------------------",
    "Get All Flask: kubectl get svc -n flask",
    "Get All Pods All Namespaces: kubectl get pods --all-namespaces -o wide",
    "Get All Monitoring Services : kubectl get svc -n monitoring",
    "Get All Monitoring Pods : kubectl get pods -n monitoring",
     "---------------------KubeCtl Cmd----------------------------------",
    "Kubeconfig Cmd: aws eks update-kubeconfig --name ${var.cluster_name} --region ${var.region}",
    "Kubeconfig Apply Auth : kubectl apply -f ${local_file.aws_auth_configmap.filename}",
     "---------------------KubeCtl Clean----------------------------------",
    "Kubeconfig Clean Flask : kubectl delete svc --all -n flask",
    "Kubeconfig Clean Monitoring : kubectl delete svc --all -n monitoring",
    "Kubeconfig Clean Auth : kubectl delete configmap aws-auth -n kube-system",
    "------------------------------------------------------------------",
    "------------------------------------------------------------------",
